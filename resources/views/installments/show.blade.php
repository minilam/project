            ...
            <div class="installment-next text-right">
              ...
              @else
                ...
                <div class="payment-buttons">
                  <a class="btn btn-primary btn-sm" href="{{ route('installments.alipay', ['installments' => $installment->id]) }}">支付宝支付</a>
    ...
    <script>
        $(document).ready(function() {
            $('#btn-wechat').click(function () {
                swal({
                    content: $('<img src="{{route('installments.wehcat', ['installment' => $installment->id])}}">')[0],
                    buttons: ['关闭', '已完成付款']
                }).then(function (result) {
                    // 如果用户点击了已完成付款，则重新加载页面
                    if (result) {
                        location.reload();
                    }
                })
