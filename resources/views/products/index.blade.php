        ...
        <!-- 输出属性名 -->
          ...
          <!-- 遍历属性表 -->
            @foreach($property['values'] as $value)
              ...
              <a href="javascript: appendFitlerToQuery('{{ $property['key']}}', '{{ $value }}');"> {{ $value }}</a>
            @endforeach
          </div>
        </div>
      @endforeach
...
<script>
  ...
  function parseSearch() {
    ...
    // substr(1) 将 ? 去除，让后以符号 & 分割成数组，然后遍历这个数组
    location.search.substr(1).split('&').forEach(function (str) {
      // 将字符串以符号 = 分割成数组
      var result = str.split('=');
      // 将数组中第一个指解码之后作为key, 第二个指解码后作为 value, 放到之前初始化的对象中
  ...
  // 将新的 filter 追加到当前的url当中
  function appendFitlerToQuery(name, value) {
    // 解析当前 Url 查询参数
    var searches = parseSearch();
    // 如果已经有了 filters 查询
    if (searches['filters']) {
