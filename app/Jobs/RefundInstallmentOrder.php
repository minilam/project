...
class RefundInstallmentOrder implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($this->order->payment_method !== 'installment'
            || !$this->order->paid_at
            || $this->order->refund_stauts !== Order::REFUND_STATUS_PROCESSING) {
            return;
        }
        ...
        switch ($item->payment_method) {
            case 'wechat':
                app('wechat_pay')->refund([
                    ...
                    'total_fee' => $item->total * 100, // 原订单金额，单位 分
            ...
            case 'alipay': 
                $ret = app('alipay')->refund([
                    ...
                    'refund_amount' => $item->base, // 分期付款的商品订单，退款只退本金
                    'out_request_no' => $refundNo // 退款订单号
                ]);
                // 根据支付宝文旦，返回值里有 sub_code 字段说明退款失败
                if ($ret->sub_code) {
                    $item->update([
